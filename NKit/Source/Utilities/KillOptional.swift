...
public func ?! <T, U, V, X, Y>(left: (T?, U?, V?, X?, Y?
    ...
    if let _ = left.0, _ = left.1, _ = left.2, _ = left.3, _ = left.4 {
        right()
    }
}

public func ?! <T, U, V, X, Y, W>(left: (T?, U?, V?, X?, Y?, W?), right: (T, U, V, X, Y, W) -> Void) {
    if let t = left.0, u = left.1, v = left.2, x = left.3, y = left.4, w = left.5 {
        right(t, u, v, x, y, w)
    }
}

public func ?! <T, U, V, X, Y, W>(left: (T?, U?, V?, X?, Y?, W?), right: () -> Void) {
    if let _ = left.0, _ = left.1, _ = left.2, _ = left.3, _ = left.4, _ = left.5 {
        right()
    }
}
